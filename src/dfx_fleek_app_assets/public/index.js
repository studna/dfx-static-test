...
const getText = (bytes) => new TextDecoder().decode(new Uint8Array(bytes));
...
const getInjectingFunc = (fileName) => {
  ...
  return (bytes) =>
    console.error(
      "This type of resource is not handled yet: ",
      fileName,
      getText(bytes)
    );
};
...
const injectResources = (jsonBytes) => {
  ...
  Object.values(json.files).forEach((filename) =>
    entrypoints.push(filename.substr(1))
  );

  console.log("scripts to load: ", entrypoints);
...
const initInjectingResources = async () => {
  ...
  const result = await dfx_fleek_app_assets.retrieve("asset-manifest.json");
